...
public:
	...
	bool operator==(Animal a1){return this->getEspecie() == a1.getEspecie() && this->getNome() == a1.getNome();}
};
...
struct hAnimal {
	int operator()(const Animal& a1) const{
		return a1.getEspecie().size() * a1.getNome().size();
	}
	bool operator()(const Animal& a1, const Animal& a2) const {
		return a1.getEspecie() == a2.getEspecie() && a1.getNome() == a2.getNome();
	}
};
